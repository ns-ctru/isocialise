...
Generalised Linear Mixed Models will be fitted using the [`lme4`](https://github.com/lme4/lme4/) package for [R](https://www.r-project.org/) to estimate the effect of LEGO therapy compared to normal treatment on the stated outcome(s).  The output of individual models will be presented along with graphical representations and diagnostic plots to assess model fit and validity (see Appendix for examples).  To allow for intra-class correlation (ICC) that may arise from the delivery of the LEGO therapy in groups random effects will be included for schools, the unit of clustering and also therapist to allow for variation introduced by having multiple therapists at the same school which may arise if there are >3 eligible participants at any given school.

Tables and graphs summarising the  baseline measurements by treatment arm and overall will be produced (see Appendix for examples), although no formal test for imbalances at baseline will be peformed as they are redundant and inflate the possibility of false positives, rather clinical knowledge of what is important to adjust for should guide selection of baseline covariates to be included in the model [@roberts1999, @senn1995, @senn1994, @altman1985].  Similar tables will also be produced summarising outcomes at follow-up of 16 and 52 weeks, although again no formal tests for statistical differences will be performed on the univariable outcomes as it provides unadjusted estimates of treatment effect which will be modified when adjusted for baseline and other covariates in the multivariable Generalised Linear Mixed Model.

In keeping with best practice [@vickers2012], assessment tools that produce scores on a numeric bounded scale will be analysed as such rather than categorising into two or more arbitrary categories (for more in problems with this approach see also [here](http://biostat.mc.vanderbilt.edu/wiki/Main/CatContinuous)), however if there is evidence of non-linearity flexibile parametric approaches will be investigated.

**ToDo** Decide methods, perhaps regression splines? input from Dawn
